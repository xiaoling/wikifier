...
//	            }
	...
	* @author xiaoling
	*/
	private static List<WikiMatchData> getAllCandidates(String mention) throws Exception{
	String surfaceForm = mention;
	int maxCandidates = GlobalParameters.params.maxCandidatesToGenerateInitially;
	...
	public static void main(String[] args) throws Exception {
		...
		wiki = GlobalParameters.wikiAccess;
		String[] lines = FileUtils.readFileToString(new File("queries")).split("\n");
	    StringBuilder sb = new StringBuilder();
	    for (String line: lines) {
	    	line  = line.trim();
		List<WikiMatchData> candidates = getAllCandidates(line);
	    	sb.append(line);
	    	for (int i = 0; i < candidates.size(); i++) {
	    		sb.append("\t"+candidates.get(i).basicTitleInfo.getTitleSurfaceForm());
	    	}
	    	sb.append("\n");
	    }
	    FileUtils.writeStringToFile(new File("queries.candidates"), sb.toString());
		System.exit(-1);
		...
		if (WordFeatures.isCapitalized(surfaceForm)) {
			...
			if (tokens.length == 2
					...
					&& !WordFeatures.isCapitalized(tokens[1])
					&& !isStopWord(tokens[0])) {
				String newSurface = tokens[0] + " "
						+ StringUtils.capitalize(tokens[1]);
				System.out.println(newSurface);
				for (WikiMatchData cand : getCandidates(surfaceForm)) {
					System.out.println(cand.matchedSurfaceForm+"\t"+cand.basicTitleInfo+"\t"+cand.semanticInfo);
				}
				System.out.println("======");
				for (WikiMatchData cand : getCandidates(newSurface)) {
					System.out.println(cand.matchedSurfaceForm+"\t"+cand.basicTitleInfo+"\t"+cand.semanticInfo);
				}
			}
			// Another form of acronym
			// if(WordFeatures.isLikelyAcronym(surfaceForm) &&
